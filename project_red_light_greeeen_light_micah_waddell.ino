/* Project 5 Part A - Controlling Traffic 
by Micah Waddell */

#define westButton 3  // define the pins that the buttons and lights are connected to:
...
void setup() {
  pinMode(westButton, INPUT);  // set up the digital I/O pins
...
void loop() {
  ...
  {
    if (trafficWest != true)  // only continue if traffic is flowing in the opposite (east) direction
    {
      ...
      // blink yellow light on the west side
  ...
  if (digitalRead(eastButton) == HIGH)  // request east->west traffic flow
  {
    if (trafficWest == true)  // only continue if traffic flow is in the opposite (west) direction
    {
      ...
      // blink yellow light on the east side
      for (int a = 0; a < 5; a++)  // blink yellow light
      {
        digitalWrite(eastYellow, LOW);
        delay(yellowBlinkTime);
